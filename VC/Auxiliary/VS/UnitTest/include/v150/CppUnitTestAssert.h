...
#define  LINE_INFO() std::unique_ptr<Microsoft::VisualStudio::CppUnitTestFramework::__LineInfo>(new Microsoft::VisualStudio::CppUnitTestFramework::__LineInfo(__WFILE__, __FUNCTION__, __LINE__)).get()
...
#if _HAS_CXX17
	template<> inline std::wstring ToString<std::wstring_view>     (const std::wstring_view& t)     { auto wstr = std::wstring{ t }; return wstr; }
	template<> inline std::wstring ToString<std::string_view>      (const std::string_view& t)      { auto str = std::string{ t }; RETURN_WIDE_STRING(str.c_str()); }
#endif
