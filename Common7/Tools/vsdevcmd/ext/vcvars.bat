...
for /F "tokens=1,2,3,* delims=." %%a in ("%__VCVARS_VERSION%") DO (
   ...
   if "%%d" NEQ "" (
       ...
       @REM TODO: AzDO Bug#1293526 - Remove this special-case in next major VS version.
       if "%__VCVARS_VERSION%" == "14.28.16.8" (
           @REM special handling of the 16.8 SxS toolset, in this case we need to use VC\Auxiliary\Build\14.28 directory and SxS files
           if "%VSCMD_DEBUG%" GEQ "1" @echo [DEBUG:ext\%~nx0] Looking up MSVC SxS Toolset version VC\Auxiliary\Build\14.28
           set "__VCVARS_VERSION=14.28"

           if NOT EXIST "%VSINSTALLDIR%\VC\Auxiliary\Build\14.28\Microsoft.VCToolsVersion.14.28.txt" (
               @echo [WARN:%~nx0] SxS version 14.28 was not present/installed, -vcvars_ver=14.28.16.8 was specified. Attempting fallback selection of toolset VC\Tools\MSVC\14.28.nnnnn.
               goto :check_vcvars_ver_major_minor
           )
       ) else if NOT EXIST "%VSINSTALLDIR%\VC\Auxiliary\Build\%__VCVARS_VERSION%\Microsoft.VCToolsVersion.%__VCVARS_VERSION%.txt" (
...
if EXIST "%VCINSTALLDIR%Auxiliary\Build\Microsoft.VCToolsVersion.v142.default.txt" (
    if "%VSCMD_DEBUG%" GEQ "2" @echo [DEBUG:ext\%~nx0] Microsoft.VCToolsVersion.v142.default.txt was found.
    set "__VCVARS_DEFAULT_CONFIG_FILE=%VCINSTALLDIR%Auxiliary\Build\Microsoft.VCToolsVersion.v142.default.txt"

) else (
    if "%VSCMD_DEBUG%" GEQ "1" @echo [DEBUG:ext\%~nx0] Microsoft.VCToolsVersion.v142.default.txt was not found. Defaulting to 'Microsoft.VCToolsVersion.default.txt' instead to check for v141.
)
