...
@REM At this point, [VS160COMNTOOLS] may not have been set, so we call
...
if "%VSCMD_TEST%" NEQ "" (
    ...
    if "%VSCMD_DEBUG%" NEQ "" (
        @echo [DEBUG:%~n0] Writing pre-initialization test environment to %temp%\test_dd_vsdevcmd16_env.log
    )
    @REM if running tests, dump the environment to [TEMP]\%test_dd_vsdevcmd16_env.log
    set > %temp%\test_dd_vsdevcmd16_env.log
...
@REM VS160COMNTOOLS set.  We save that value, so we can restore this
@REM variable on -clean_env.
if "%VS160COMNTOOLS%" NEQ "" (
    if "%__VSCMD_PREINIT_VS160COMNTOOLS%" == "" (
        set "__VSCMD_PREINIT_VS160COMNTOOLS=%VS160COMNTOOLS%"
    )
)

@REM using pushd/popd to avoid having "..\" in the env var.
@REM Set VS160COMNTOOLS to scripts root
@REM Go from Tools/vsdevcmd/core to Tools
pushd "%~dp0..\..\"
set "VS160COMNTOOLS=%CD%\"
popd

if "%VS160COMNTOOLS%" NEQ "" set "PATH=%VS160COMNTOOLS%;%PATH%"

:set_vsinstalldir

if "%VSINSTALLDIR%" NEQ "" goto :set_devenvdir
@REM Set VSINSTALLDIR to Visual Studio Installation Directory
@REM vsdevcmd_start.bat location: Microsoft Visual Studio 16.0\Common7\Tools\vsdevcmd\core
@REM VSINSTALLDIR location: Microsoft Visual Studio 16.0
pushd "%~dp0..\..\..\..\"
set "VSINSTALLDIR=%CD%\"
popd
...
:end
if "%VSCMD_DEBUG%" GEQ "2" (
    @echo [DEBUG:%~n0] end of script: VS160COMNTOOLS="%VS160COMNTOOLS%"
