...
@REM Restore the "classic installer" VS160COMNTOOLS value, if it was set prior
@REM to command prompt init.
if "%__VSCMD_PREINIT_VS160COMNTOOLS%" NEQ "" (
    set "VS160COMNTOOLS=%__VSCMD_PREINIT_VS160COMNTOOLS%"
) else (
    set VS160COMNTOOLS=
)
...
if "%VSCMD_DEBUG%" NEQ "" (
    @echo [DEBUG:%~n0] Writing post-clean environment to %temp%\dd_vsdevcmd16_postclean_env.log
    set > "%temp%\dd_vsdevcmd16_postclean_env.log"
)
